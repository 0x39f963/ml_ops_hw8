...
Я подготовил Python-окружение для запуска скриптов ДЗ.

Создал `.venv`, обновил `pip`, запустил установку зависимостей.

![создание окружения](1.png)

Зависимости поставились нормально. После этого запускаются проверка сервиса,
drift, диаграмма и стрим-демо.

![установка зависимостей](2.png)

## Запуск контейнеров

Поднял весь стенд одной командой через `docker compose up -d`.

В списке контейнеров видны все сервисы: ML-сервис, Prometheus, Grafana,
PostgreSQL, DQOps и Redpanda.
